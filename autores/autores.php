...
if ($_POST) {
    if (!empty($_POST['original'])) {
        ...
        $conexion->prepare($sql)->execute([
            $_POST['DNI'], $_POST['nombre'], $_POST['nacionalidad'], $_POST['original']
        ]);
    } else {
        $sql = "INSERT INTO autores (DNI, nombre, nacionalidad) VALUES (?,?,?)";
        $conexion->prepare($sql)->execute([
            $_POST['DNI'], $_POST['nombre'], $_POST['nacionalidad']
        ]);
    }
    header("Location: autores.php");
    exit;
...
<div class="container mt-4">
    ...
    <table class="table table-bordered table-striped">
        <thead class="table-dark">
            <tr><th>DNI</th><th>Nombre</th><th>Nacionalidad</th><th>Acciones</th></tr>
        </thead>
        <tbody>
